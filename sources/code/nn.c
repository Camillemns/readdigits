...
net* init_neural_network (int in, int hid, int out)
{
    ...
    for (int i = 0; i < nn -> nbr_hidden; i++)
    {
        ...
        {
            nn -> w_input_to_hidden[j][i] = randomizer();
        }
        nn -> bias_input_to_hidden[i] = randomizer();
    }
    ...
    for (int i = 0; i < nn -> nbr_output; i++)
    {
        ...
        {
            nn -> w_hidden_to_output[j][i] = randomizer();
        }
        nn -> bias_hidden_to_output[i] = randomizer();
    }
    return nn;
}
...
void free_nn(net *a)
{
    free(a->error_rate_hid);
    free(a->error_rate_out);
    free(a->goal);
    free(a->output);
    free(a->dbHO);
    ...
    free(a -> dwHO);
    free(a -> bias_hidden_to_output);
    for (int i = 0; i < a -> nbr_hidden; i++)
    {
        free(a -> w_hidden_to_output[i]);
    }
    free(a -> w_hidden_to_output);
    free(a -> hidden);
    free(a -> dbIH);
    ...
    free(a -> dwIH);
    free(a -> bias_input_to_hidden);
    for (int i = 0; i < a -> nbr_input; i++)
    {
        free(a -> w_input_to_hidden[i]);
    }
    free(a -> w_input_to_hidden);
    free(a -> input);
}
